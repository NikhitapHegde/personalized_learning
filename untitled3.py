# -*- coding: utf-8 -*-
"""
Created on Sat Dec  9 12:56:55 2023
...
def get_question():
    global progress
    difficulty_factor = sigmoid(progress - 2)  # Adjust the difficulty based on progress
    x_range = int(start_index[-1] * difficulty_factor)
    y_range = int(end_index[-1] * difficulty_factor)
    ...
    return random.choice(questions[x_range:y_range])
import csv
import random
import math
...
def main():
    ...
    while progress < 5:
        ...
        while attempts < max_attempts:
            ...
            if evaluate_answer(user_answer,int( current_question[2])):
                break
            else:
                attempts += 1
